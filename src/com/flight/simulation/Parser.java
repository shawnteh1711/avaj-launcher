...
public class Parser {
    ...
    public static ParsedData parseScenarioFile(String scenarioFile) throws ScenarioParseException {
        ...
        int simulationSteps = parseSimulationSteps(fileLines.get(0), 1);
    ...
    private static AircraftData parseAircraftData(String line, int lineNumber) throws ScenarioParseException {
        ...
        if (parts.length != 5) {
            throw new ScenarioParseException(
                "Invalid line format. Expected: TYPE NAME LONGITUDE LATITUDE HEIGHT", 
                lineNumber,
                line
            );        
        }
        ...
        try {
            ...
            throw new ScenarioParseException(
                "Invalid coordinate format. Coordinates must be integers.", 
                lineNumber,
                line,
                e
            );        
    ...
    private static int parseSimulationSteps(String line, int lineNumber) throws ScenarioParseException {
        try {
            int steps = Integer.parseInt(line);
            if (steps <= 0) {
                throw new ScenarioParseException(
                    "Simulation steps must be a positive integer.", 
                    lineNumber,
                    line
                );
            }
            ...
            throw new ScenarioParseException(
                "Invalid number format for simulation steps. Must be an integer.", 
                lineNumber,
                line,
                e
            );
    ...
    private static void validateAircraftType(String aircraftType, int lineNumber, String line) throws ScenarioParseException {
        if (!aircraftType.equals("Helicopter") && !aircraftType.equals("JetPlane") && !aircraftType.equals("Baloon")) {
            throw new ScenarioParseException(
                "Invalid aircraft type '" + aircraftType + "'. Must be Helicopter, JetPlane, or Baloon.", 
                lineNumber,
                line
            );        
        }
    ...
    private static void validateAircraftName(String aircraftName, int lineNumber, String line) throws ScenarioParseException{
        if (!aircraftName.matches("[a-zA-Z0-9_]+")) {
            throw new ScenarioParseException(
                "Invalid aircraft name '" + aircraftName + "'. Only letters, numbers, and underscores allowed.", 
                lineNumber,
                line
            );        
        }
    ...
    private static void validateCoordinates(int longitude, int latitude, int height, int lineNumber, String line) throws ScenarioParseException {
        if (longitude < 0 || latitude < 0 || height < 0) {
            throw new ScenarioParseException(
                "Negative coordinate values not allowed. Longitude: " + longitude + ", Latitude: " + latitude + ", Height: " + height, 
                lineNumber,
                line
            );        
        }
        if (height > 100) {
            throw new ScenarioParseException(
                "Height exceeds maximum limit of 100. Current height: " + height, 
                lineNumber,
                line
            );        
        }
